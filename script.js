...
'bangunan': { type: 'geojson', sourceId: 'bangunan-source', geojsonUrl: './source/data/bangunan_tegalsari.geojson', layerId: 'bangunan-layer', legendTitle: 'Bangunan', items: { 'Area Bangunan': { type: 'fill-outline', color: '#E67E22' } } },
        ...
        // Kategori diturunkan dari field Sub_Unsur (dicocokkan dengan Toponim yang ada sebagai bukti),
        // BUKAN dari tabel kode resmi BIG/RBI — domain code lookup-nya gak ikut ke-export di shapefile.
        // Kalau nanti dapat tabel kode resminya, tinggal update KATEGORI_MAP pas generate geojson-nya.
        'sapras': { type: 'geojson', sourceId: 'sapras-source', geojsonUrl: './source/data/sapras_tegalsari.geojson', layerId: 'sapras-layer', labelLayerId: 'sapras-label-layer', legendTitle: 'Sarana & Prasarana', property: 'Kategori', items: {
            'Pendidikan': { type: 'circle', color: '#3b82c4' },
            'Peribadatan': { type: 'circle', color: '#5a8a5a' },
            'Pemerintahan & Sosial': { type: 'circle', color: '#8a6d3b' },
            'Ekonomi & Perdagangan': { type: 'circle', color: '#d68a2c' },
            'Fasilitas Umum Lainnya': { type: 'circle', color: '#b5737a' }
        } },
'orientasi-bangunan': {
    type: 'geojson',
    ...
    safeAdd('sapras', () => {
        ...
        if (sprConfig && map.getSource(sprConfig.sourceId) && !map.getLayer(sprConfig.layerId)) {
            map.addLayer({
                ...
                paint: {
                    'circle-radius': 6,
                    'circle-color': ['match', ['get', sprConfig.property], ...Object.entries(sprConfig.items).flatMap(([k, v]) => [k, v.color]), '#999'],
        ...
        map.on('click', sprConfig.layerId, (e) => {
            ...
            const html = `
                <div style="font-family:'Poppins',sans-serif; min-width:180px;">
                    ...
                    <table style="width:100%; font-size:12.5px; border-collapse:collapse;">
                        <tr><td style="padding:2px 0;">Kategori</td><td style="text-align:right; font-weight:600;">${kategori}</td></tr>
